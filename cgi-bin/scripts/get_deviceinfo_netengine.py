...
class session_create_netengine(interfaceinfo):
    """
    ostype が netengine の時にデバイス情報を取得するクラス
    """
    ...
    def get_sysinfo(self):
        ...
        for row in stdout_list:
            if row.startswith("ShfName="): model_row = row   # モデル名を含む行のみを抽出
        model = model_row.split("=")[1]       # モデル名のみを抽出
        self.model = model.replace("\r", "").replace("<", "").replace(">", "")

        ### 筐体シリアルナンバーを取得
        for row in stdout_list:
            if row.startswith("BarCode="): serial_row = row   # シリアルを含む行のみを抽出
        serial = serial_row.split("=")[1]       # シリアル番号のみを抽出
        self.serial = serial.replace("\r", "")
